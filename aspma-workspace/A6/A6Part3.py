...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.2, t2=0.4, window='hamming', 
    ...
    intervalStart = np.ceil(t1*float(fs)/H)
    intervalEnd = np.ceil(t2*float(fs)/H)
    intervalHarmonics = hfreq[intervalStart:intervalEnd]
    intervalf0 = f0[intervalStart:intervalEnd]

    # 3. Compute the mean inharmonicity of the segment
    meanInharm = np.array([])	
    for z, n in enumerate(intervalHarmonics):
        y = np.arange(1, n.size+1)
        i = np.abs(n - y * intervalf0[z])/y
        meanInharm = np.append(meanInharm, np.sum(i) / len(i))
   
    totalInharm = np.sum(meanInharm)/len(meanInharm)
    return totalInharm
