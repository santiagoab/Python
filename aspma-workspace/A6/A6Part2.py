...
minf0=310, maxf0=650):
    ...
    filteredSegment = []
    for i in segments:
    	if (i[1] - i[0])*H/float(fs)>=minNoteDur:
    		filteredSegment.append(i)

    # plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)
    segments = np.array(filteredSegment)
    plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)
