...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    freqs = np.arange(N/2+1)*float(fs)/N

    lowend = int(np.ceil(float(3000)*N/fs))
    highend = int(np.ceil(float(10000)*N/fs))
    low = mX[:,1:lowend]
    high = mX[:,lowend:highend]

    ln1 = np.power(10, low/20.0)
    energylow = 10*np.log10(np.sum(ln1*ln1, axis=1))
    ln2 = np.power(10, high/20.0)
    energyhigh= 10*np.log10(np.sum(ln2*ln2, axis=1))
