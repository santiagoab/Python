...
def minFreqEstErr(inputFile, f):
    ...
    for i in range (1,50)
       window lenght M = 100*i +1
       DTF size:first power of 2>M
       w = get_window(window,N)
       extract a part of the signal (middle frame)
       compute dtf.dftAnal()
       Detect peak in the magnitude spectrum UF.peak.Detection()
       Refine peak values UF.peakInterp()
       fEst = ...
       if abs(fEst-f) < 0.05
          break
    return fEst, M, N
